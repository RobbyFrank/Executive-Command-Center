...
3. **Review staleness** — `getReviewStaleWindowHours` / `isReviewStale(..., ownerAutonomy)`; Roadmap **Need review** and the Review page use owner autonomy from Team. Project rows pulse the **⋯** control when the project is on the Review queue (same stale rule).
