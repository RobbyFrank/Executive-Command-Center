...
2. **Review** — The server loads **top-level** channel messages only (`conversations.history`, user token), caps transcript size for the model, and asks Claude to return a JSON **array** of suggestions. The system prompt includes the **Team roster** (person id, name, Slack user id) so the model can set `ownerPersonId` on new goals and `assigneePersonId` on projects when Slack clearly assigns someone. After parsing, the server **enriches** suggestions: it sets each **new goal’s** `slackChannel` / `slackChannelId` from the evidence channels (first resolvable channel name → id), and fills missing owners/assignees from `@mentions` in evidence quotes or the **message author** line (`user_or_bot` in the transcript). You **multi-select** which proposed **goals** (with optional nested **projects**) and which **projects on existing goals** to import. **Add selected** writes everything in **one** tracker commit (goal owner, project assignees, and goal Slack channel are persisted when valid).
