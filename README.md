...
- **Roadmap Slack sync** — Vercel Cron calls `GET /api/cron/slack-roadmap-sync` once daily (UTC midnight, see `vercel.json`) to scan each company’s Slack (threaded history, 2-day window) and refresh the **pending review** queue in Redis (`ecc:slackSuggestions:data`). The transcript sent to the model includes **human-authored messages only** (bot-only webhooks are dropped) so automated billing/alerts don’t spawn goals. Same **`CRON_SECRET`** auth and Slack user token as other Slack features. The **`/sync`** page lists pending items with **company + type filters**, **collapsible company groups** (with logos), **Approve all** (with confirmation), per-card **Reject / Revise with AI / Approve**, and streams **Revise** via `POST /api/slack-suggestions/revise`. **Troubleshooting in Vercel:** runtime logs are structured JSON; filter by **`ecc:slackRoadmapSync`**. See [docs/roadmap-slack-scrape.md](docs/roadmap-slack-scrape.md) and [docs/operations.md](docs/operations.md#roadmap-slack-scan-cron).
