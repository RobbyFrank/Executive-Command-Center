...
- **Roadmap Slack sync** — Vercel Cron calls `GET /api/cron/slack-roadmap-sync` once daily (UTC midnight, see `vercel.json`) to scan each company’s Slack (threaded history, 2-day window) and refresh the **pending review** queue in Redis (`ecc:slackSuggestions:data`). Same **`CRON_SECRET`** auth and Slack user token as other Slack features. The Roadmap **scan** button and nav badge surface items for **approve/reject** in-app. **Troubleshooting in Vercel:** runtime logs are structured JSON; filter by **`ecc:slackRoadmapSync`**. See [docs/roadmap-slack-scrape.md](docs/roadmap-slack-scrape.md) and [docs/operations.md](docs/operations.md#roadmap-slack-scan-cron).
